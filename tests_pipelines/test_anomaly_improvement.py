...
def test_alert_grouping_logic_explanation():
    ...
    allure.attach(
        ...
        f"Noise (Sensor Jitter):\n"
        f"├─ Affects: 1 reading\n"
        f"├─ Pattern: [99, 98, 200, 101, 99] ← spike at index 2\n"
        f"├─ Treatment: Filtered (isolated spike)\n"
        f"└─ Result: Not escalated\n\n"
        f"Real Issues (System Problem):\n"
        f"├─ Affects: 2+ consecutive readings\n"
        f"├─ Pattern: [99, 98, 200, 195, 205, 100] ← spikes at 2,3,4\n"
        f"├─ Treatment: Kept (consecutive spikes = real issue)\n"
        f"└─ Result: Escalated\n\n"
        f"TEST VALIDATION:\n"
        f"Test 1 (Isolated spike):\n"
        f"  Flags before: {flags_before} → Flags after: {flags_after}\n"
        f"  Expected: {flags_before} → 0 (noise filtered)\n"
        f"  Result: {'✓ PASS' if flags_after == 0 else '✗ FAIL'}\n\n"
        f"Test 2 (Consecutive spikes):\n"
        f"  Flags before: {flags_before2} → Flags after: {flags_after2}\n"
        f"  Expected: 3 → 3 (real issue kept)\n"
        f"  Result: {'✓ PASS' if flags_after2 == 3 else '✗ FAIL'}\n\n"
        f"UNDERSTANDING DEMONSTRATED:\n"
        f"✓ Know difference between noise (1-point) vs issues (2+-point)\n"
        f"✓ Understand why grouping filters false positives\n"
        f"✓ Can explain the trade-off (fewer alerts, but catches real issues)\n"
        f"✓ Can validate logic with targeted test cases",
